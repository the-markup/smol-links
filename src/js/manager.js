...
class ShlinkifyManager {
	...
	load() {
		...
		return fetch(
			`/wp-admin/admin-ajax.php?action=shlinkify_load&tab=${ tab }&_wpnonce=${ shlinkify_nonces.load }`
		);
	}
	...
	clickHandler( event ) {
		...
		let copy =
			event.target.classList.contains( 'shlinkify-item__copy' ) ||
		...
		if ( item.querySelector( '.shlinkify-item__edit' ) ) {
			...
			form.innerHTML = `
					...
					<div class="shlinkify-edit-field">
						...
						<input type="text" id="shlinkify-edit-title" name="title" class="shlinkify-title regular-text ltr" value="${ item.getAttribute(
							'data-title'
						) }">
					</div>
					<div class="shlinkify-edit-field">
						<label for="shlinkify-edit-long-url" class="shlinkify-label">Long URL</label>
						<input type="text" id="shlinkify-edit-long-url" name="long_url" class="shlinkify-long-url regular-text ltr" value="${ item.getAttribute(
							'data-long-url'
						) }">
					</div>
